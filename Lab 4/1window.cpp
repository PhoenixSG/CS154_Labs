...
window = new Fl_Window (800,600,"Changed Size and Label"); // outer window

		Fl_Button *buttons[4][4];

		for(int i=0; i<4; i++){
			for(int j=0; j<4; j++){
				buttons[i][j]=new Fl_Button(100*i+150,100*j+100,100,100,charstar_from_int(4*j+i+1));
			}
		}

		window->color(FL_WHITE);
		buttons[3][3]->label("");
		buttons[3][3]->down_color(FL_RED);

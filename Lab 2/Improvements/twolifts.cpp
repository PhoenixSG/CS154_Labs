...
Mar 17, 2021
Improvements in Lab 2, CS154

Demonstrating two lifts and their interaction with the people as they board and leave at different floors.
Using OOP Paradigm.
Using lift, floor and person class as abstraction.
*/
...
class lift{
    ...
    int person::which_lift(){
        return lift;
    }

    void person::set_lift(int input){
        lift=input;
    }
    ...
    void lift::add_person(person* input, int index, string* namelist){
        if(no_of_people<5){
            ...
            no_of_people++;
            cout<<"Welcome to the lift "<<number<<" "<<namelist[index]<<".\n";
        }
        else{
            cout<<"Sorry "<<namelist[index]<<"! The lift "<<number<<" is full right now, please come back later\n";
        }
    }
    ...
    void lift::floor_management(person* people, int index, string* namelist){
        for(int i=0; i<no_of_people; i++){
            if(list[i]->get_destination()==floor && get_number()==list[i]->which_lift()){
    ...
    void lift::show(){
        cout<<"The number of people in the lift "<<number<<" are "<<no_of_people<<"\n";
        cout<<"The lift "<<number<<" is currently on floor "<<floor<<" and moving ";
...
int main(){
    ...
    lift l1(0,1,1), l2(6,-1,2);           //floor and direction
    person people[100];
    string namelist[70];    
    ...
    l2.floor_management(people, index+1, namelist);
    l1.show();
    l2.show();
    cin>>c;
